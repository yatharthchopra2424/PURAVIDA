...
export default function ContactPage() {
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      try {
        // Was /api/catalog/products, which requires a `category` param
        // and returned 400 — so this list was always empty.
        const response = await fetch("/api/catalog/product-names");
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Surface the server's reason — rate limiting and validation
      // failures are actionable, so a generic "something went wrong"
      ...
      try {
        ...
        if (payload.fields) {
          message = Object.values(payload.fields)[0] ?? message;
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1fr_380px]">
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="mt-10 space-y-6">
              {/*
                Honeypot. Hidden from sighted users and from screen
                readers (aria-hidden + tabIndex -1), so only a bot that
                fills every input will populate it.
              */}
              <div
                aria-hidden="true"
                style={{
